...
public class TcpFrameParser implements Handler<Buffer> {

	  private Buffer _buffer;
	  private int _offset;

	  private final Handler<AsyncResult<TcpRequest>> client;

	  public TcpFrameParser(Handler<AsyncResult<TcpRequest>> client) {
	    this.client = client;
	  }
	  ...
	  public void handle(Buffer buffer) {
	    ...
	    while (true) {
	      ...
	      if (remainingBytes - 4 >= length) {
	        ...
	        try {
	          client.handle(Future.succeededFuture(new TcpRequest(_buffer,_offset, length)));
	        } catch (Exception e) {
	          // bad json
