...
import org.lmars.dm.storage.TopicFieldType;
import org.lmars.dm.storage.TopicMapping;

/*[driver]
type=h2
url=jdbc:h2:C:\GISCloudPlatform\GISBaseService\DMessage\MsgBufDB\testdb3*/

public class H2Driver implements IStorgeDriver{
	...
	public void init(Map<String, String> conf,Map<String, TopicMapping> topics) throws Exception{
		...
		String db_url = conf.get(Config.driver_section_h2_url);
        ...
        for(TopicMapping topic : topics.values()){
        	...
        	for(int i = 0;i<fs.size();i++){
        		...
        		switch(f.ftype){
        		case t_string:
        			sb.append(f.db_name).append(" VARCHAR  ");
        			break;
        		case t_point:
        			...
        			break;
        		case t_time:
        			sb.append(f.db_name).append(" BIGINT ");
        			break;
        		case t_int:
        			sb.append(f.db_name).append(" INT ");
        			break;
        		case t_float:
        			sb.append(f.db_name).append(" REAL ");
        			break;
        		case t_double:
        	...
        	try(Connection conn = cp.getConnection();Statement stat = conn.createStatement();){
        	...
        	for(int i = 0;i<fs.size();i++){
        		...
        		if(f.index_flag){
            		...
            		case t_point:
                    	...
                    	try(Connection conn = cp.getConnection();Statement stat1 = conn.createStatement();Statement stat2 = conn.createStatement();){
                    		stat1.execute(sql1);        		
                    		stat2.execute(sql2);        		
                    	}catch(Exception e){
		...
		try(Connection conn = cp.getConnection();Statement stat = conn.createStatement();){
			...
			for(StreamMessage msg : msgs){
				try{
					...
					sb_keys.append("insert into ").append(topic_map.table).append(" (");
